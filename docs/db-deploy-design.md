...
页面与 API：`templates/dbmgr/deploy_job_*.html`；路由 `apps/dbmgr/urls.py`（`db-deploy/`）。
...
`install.*` 路径由 `finalize_mysql_deploy_params()` 按端口生成（如 `/data/mysql3306`），用户一般无需填写。共享二进制目录固定 `/usr/local/mysql`。
...
| `instance_root` / `datadir` | `/data/mysql{port}` | Oracle 路径见 Profile | 由 `build_mysql_install_paths(port)` 生成 |
| `log_error` / `binlog_dir` | `{instance_root}/mysql_err.log`、`{instance_root}/binlog` | — | 由 `build_mysql_install_paths` 生成 |
...
# MySQL only:
merged["install"] ← build_mysql_install_paths(port)  # /data/mysql{port}/...
...
- Profile 的 `default_params` 用于预填端口、binlog/gtid、内存等
- `connect_host` 默认展示目标主机业务 IP；MySQL 须与业务 IP 一致（后端强制）
- 高级项：binlog/gtid、innodb_buffer_pool_size、DBA 账号
...
| enable_binlog / enable_gtid | 配置 | 否 | Profile 默认 true | my.cnf |
| datadir 等 install 路径 | 安装 | 自动 | `build_mysql_install_paths` | 用户不填 |
...
| **环境级默认值** | 如「生产环境默认 charset=utf8mb4」；实例路径由端口派生，非环境级 |
...
**待第二期**：`mysql_replica`、MGR、RAC；Profile 落库管理页；部署后自动 probe。
...
2. 选 Profile 后表单预填 `default_params`（端口、binlog/gtid、内存等）；MySQL 实例路径由后端按端口生成，表单不展示 datadir。
...
4. configure / initialize / start 等后续 tag 处理实例目录（`/data/mysql{port}`）。
...
*文档版本：v1.8 · 全文对照代码校准（模型/编排/API/参数/Profile/待办）· Django 5.2 + LayUI + Celery + Ansible*
