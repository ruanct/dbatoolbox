...
> **状态**：草案 v0.1  
> **适用范围**：dbatoolbox 实例部署 — MySQL 单实例（`mysql_standalone`）及版本档案（Profile YAML）  
> **关联**：MySQL **运行参数模板**（`DbDeployMysqlParamTemplate`）见 [db-deploy-design.md](./db-deploy-design.md) §4.10。  
...
3. Ansible 根据 `resolved_params` 生成 `my.cnf`（**当前**：`site.yml` 内联按 major 分支；**规划**：Jinja2 模板，见 [db-deploy-mysql-multiversion-tasks.md](./db-deploy-mysql-multiversion-tasks.md)）。
...
│       ├── 5.7.44.yml          # 已上线
│       ├── 8.0.45.yml          # 已上线
│       ├── 8.0.45-glibc217.yml # 可选变体
│       └── 8.4.xx.yml          # 后续
├── templates/
│   └── mysql/
│       └── my.cnf.j2           # 规划（尚未落地；configure 仍内联于 site.yml）
└── playbooks/
...
| `profile_code` | 全局唯一，建议 `mysql-{major}.{minor}`，如 `mysql-8.0.45` |
...
> `instance_root`、`datadir`、`socket`、`cnf_path`、`log_error`、`tmpdir`、`slow_query_log_file` 等由 `build_mysql_install_paths(port)` 派生（实例根目录 `/data/mysql/db{port}`），**不要**写在 Profile 里。
...
- `basedir`、`datadir`、`port`、`socket`、`log-error`、`tmpdir`、`slow_query_log_file` 路径
- `server_id`、`log_bin` 路径（binlog 开启时由 `finalize_mysql_deploy_params` 生成）

上述键亦列入 `MYSQL_PARAM_TEMPLATE_RESERVED_NAMES`，**运行参数模板**中不可维护。
...
将 `site.yml` configure 步骤中的内联配置迁出为独立 Jinja2 模板（**尚未实施**）。
...
见 `deploy/profiles/mysql/8.0.45.yml`（`tar.xz`、`caching_sha2_password`、`utf8mb4_0900_ai_ci`）。同 major 另有 `8.0.45-glibc217.yml` 变体。
...
profile.default_params + user_params
        ↓ apply_mysql_param_template_to_merged()   # 可选：meta.mysql_param_template_title
        ↓ _deep_merge()
        ↓ finalize_mysql_deploy_params()   # apps/dbmgr/deploy_constants.py
        ↓ build_mysql_cnf_sections()       # config.cnf_sections
resolved_params（快照写入 DbDeployJob）
        ↓ extra-vars deploy=@json
ansible-playbook site.yml --tags configure
        ↓ site.yml copy 写入 cnf_sections（规划：template + my.cnf.j2）
目标机 {{ cnf_path }}  # 如 /data/mysql/db3306/my.cnf
```
...
| v0.3 | 2026-07-07 | 实例路径 `/data/mysql/db{port}`；参数模板合并链路；保留项补充 tmpdir/slow_query_log_file |
