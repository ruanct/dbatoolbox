...
> **关联文档**：[db-deploy-design.md](./db-deploy-design.md)、[db-deploy-mysql-profile-spec.md](./db-deploy-mysql-profile-spec.md)、[db-deploy-mysql5.7-step-action.md](./db-deploy-mysql5.7-step-action.md)、[MySQL8.0_vs_5.7_复制差异.md](./MySQL8.0_vs_5.7_复制差异.md)
...
| 实例目录 | `/data/mysql{从库port}`（如 `/data/mysql3306`、`/data/mysql3307`） |
...
| **从库端口** `slave_port` | 用户表单 `cmdb.port` | 从库 `my.cnf` 的 `port`、`/data/mysql{port}` 路径、`mysqld{port}` 服务、`server_id` 计算 |
...
**第一期从库任务不要求** 表单填写 `credentials.root_password` / `admin_account`（无 `post_config`；账号自主库继承）。
...
6. 若指定 `dump_account_id`，须为主库实例上的 `user_dba`
7. 从库 `port` 合法（1024～65535）
8. 从库 `connect_host` + `port` + `db_name` 不与 CMDB 已有实例冲突
9. 从库 `connect_host` + `port` ≠ 主库 `connect_host` + `port`
10. **同机**：`target_host_id` == 主库部署 `host_id` 时，从库 `port` ≠ 主库 `port`
11. 从库 `instance_name` 不重名
12. `bootstrap_method=clone` 时：主库 major ≥ 8.0（且建议 ≥ 8.0.17）
...
**须在 `mysqldump` 与 `CHANGE MASTER` 之前完成。** 均在从库主机执行，目标地址为 §2.3 的 `master_host:master_port`。
...
以下路径与检测均使用 **从库端口** `{slave_port}`（用户表单 `cmdb.port`）。示例：主库 `3306`、从库 `3307` 时，实例根目录为 `/data/mysql3307`。
...
检查 `/data/mysql{slave_port}/data`：

- 目录存在且 **目录内有文件** → 失败，消息示例：  
  `从库主机在目录 /data/mysql3307/data 上存在实例文件，请先删除实例文件。`  
  提示 DBA 在主机执行：`rm -rf /data/mysql3307/*`（或按需清理整个实例根目录）

- 第二期：任务参数 `context.force_rebuild=true` 时，可由 Ansible 在确认后自动清理

建议同时检查 `/data/mysql{slave_port}/my.cnf`、`binlog/` 等残留，避免半拉实例。  
**注意：** 同机部署时，切勿清理主库目录 `/data/mysql{master_port}/`。
...
- `/data/mysql{slave_port}`
- `/data/mysql{slave_port}/data`
- `/data/mysql{slave_port}/binlog`

并确保系统用户 `mysql` / 组 `mysql` 存在（复用单实例 `prepare`）。

### 5.4 从库运行参数（configure）

在 **用户选择的全量方案执行之前**，生成 `/data/mysql{slave_port}/my.cnf`。
...
|----|------------------------------|
| 实例目录 | `/data/mysql3307` |
| 参数文件 | `/data/mysql3307/my.cnf` |
| 数据目录 | `/data/mysql3307/data` |
| Socket | `/data/mysql3307/mysql.sock` |
| 错误日志 | `/data/mysql3307/mysql_err.log` |
| Binlog | `/data/mysql3307/binlog/mysql-bin` |
...
```bash
mysqld --defaults-file=/data/mysql{slave_port}/my.cnf --initialize-insecure --user=mysql
...
repl_bootstrap:
  ...
  mysql -S /data/mysql{slave_port}/mysql.sock -uroot < /tmp/mysql_replica_<job_id>.sql
  删除临时文件
```

**要点：**

- **不在此前/此后执行 `post_config`**；导入后从库账号表与主库一致
- `--set-gtid-purged=ON` 要求主库 GTID ON（§2.4）
- 密码通过 `defaults-extra-file` 传递，避免命令行泄露
- 导入失败 → 清理 `/data/mysql{slave_port}/*` 或 `force_rebuild` 后重试
...
repl_bootstrap:
  ...
  清空 /data/mysql{slave_port}/datadir
  ...
  chown -R mysql:mysql /data/mysql{slave_port}/data
  systemctl start mysqld{slave_port}
...
repl_bootstrap:
  ...
  （自动覆盖从库 datadir，须为 /data/mysql{slave_port}/data）
  必要时重启 / post_config
```
...
按主库 major 选择语法；**`MASTER_HOST` / `SOURCE_HOST` = `context.master_host`**（同机为 `127.0.0.1`），**`MASTER_PORT` / `SOURCE_PORT` = `context.master_port`**。
...
| 8 | `repl_setup` | §7.1 | `master_host` 同 §2.3 |
| 9 | `repl_readonly` | §7.2 | |
| 10 | `repl_verify` | §7.3 | |
| 11 | `verify` | 版本 | |
...
| `apps/dbmgr/deploy_services.py` | 创建校验、`register_instance_from_job` |
| `apps/dbmgr/profile_loader.py` | `resolve_deploy_params`、主库 context 合并 |
| `apps/dbmgr/deploy_executors/` | `MysqlReplicaExecutor`（待增） |
| `deploy/playbooks/mysql/standalone/site.yml` | 扩展 `repl_*` tag |
...
- [ ] `resolve`：§2.3 `master_host`/`master_port`、`dump_account`、§2.5 `config` 合并
- [ ] precheck：GTID ON、§2.5 白名单、§5.2 repl/DBA 异机/同机连通
- [ ] 从库路径：**无 post_config**；`initialize → start → mysqldump import`
- [ ] `repl_setup` 使用 §2.3 的 `master_host`（同机 `127.0.0.1`）
...
- [ ] 从库路径：`initialize → start`（无 post_config）→ mysqldump（主库 DBA）→ 本地 root 空密码导入
