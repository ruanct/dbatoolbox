...
| 7 | `post_config` | 后置配置 | root 密码、DBA 账号（`dba_admin`）、semi-sync master（Binlog 开启时）、bind-address→0.0.0.0 |
...
| **实例层**（按端口隔离） | 本任务数据与配置 | `/data/mysql/db3306/`（`my.cnf`、`data/`、`tmp/`、`binlog/`、`slow_query.log`、`mysqld3306.service`） |
| **网络层** | 本任务监听端口 | `cmdb.port`（如 3306） |

| 项 | 路径 |
|----|------|
| 实例根目录 | `/data/mysql/db3306` |
| 参数文件 | `/data/mysql/db3306/my.cnf` |
| 数据目录 | `/data/mysql/db3306/data` |
| 临时目录 | `/data/mysql/db3306/tmp` |
| Socket | `/data/mysql/db3306/mysql.sock` |
| 错误日志 | `/data/mysql/db3306/mysql_err.log` |
| 慢日志 | `/data/mysql/db3306/slow_query.log` |
...
| 10 | 创建实例目录 | `{{ instance_root }}`、`{{ datadir }}`、`{{ mysql_tmpdir }}`，`mysql:mysql`，`0750` |
...
由 `build_mysql_cnf_sections()` 在 `finalize_mysql_deploy_params()` 中生成 `config.cnf_sections`（平台固定项 + 可选参数模板项），`site.yml` `[configure]` 以 `copy` 写入 `cnf_path`。

要点：`bind-address=127.0.0.1`（引导期）；含 `tmpdir`、`slow_query_log_file` 等派生路径；5.7/8.0 按 `profile.major_version` 分支 collation、认证插件；Binlog/GTID 条件块由 `enable_binlog` / `enable_gtid` 控制。
...
- **Binlog 开启时**：安装 `rpl_semi_sync_master` 插件并 `SET GLOBAL`；`my.cnf` 写入 `rpl_semi_sync_master_timeout=1000`、`rpl_semi_sync_master_enabled=1`
- 创建 DBA 账号（默认名 `dba_admin`，`dba_grant_statements`）
- `bind-address` → `0.0.0.0` 并 restart（与 semi-sync cnf 变更合并为一次重启）
